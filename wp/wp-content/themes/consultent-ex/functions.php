...
function consultent_setup() {
	
	//Register a theme image size.
	add_theme_support( 'post-thumbnails' );
	add_image_size( 'consultent-large-thumb', 830 );
	add_image_size( 'consultent-medium-thumb', 550, 400, true );
	add_image_size( 'consultent-small-thumb', 230 );
	
	//Register nav menu used in theme.
	register_nav_menus( array(
		'primary' 	=> __( 'Primary Menu', 'consultent-ex' )
	) );
...
add_action( 'after_setup_theme', 'consultent_setup' );

if ( ! isset( $content_width ) ) $content_width = 900;
/*--- Blog layout ---*/
function consultent_blog_layout() {
...
function consultent_posted_on() {
	$layout = consultent_blog_layout();
...
function consultent_excerpt_more($more) {
	global $post;
	$layout = consultent_blog_layout();
	return ' ....&nbsp;&nbsp;<a class=" '.$layout.'" href="'. get_permalink($post->ID) . '">' . __('Read More', 'consultent-ex') . '</a>';
}
add_filter('excerpt_more', 'consultent_excerpt_more');


/*--- Enqueue scripts and styles ---*/
function consultent_scripts() {
	...
	wp_enqueue_script( 'consultent-common', get_template_directory_uri() . '/js/common.js', array('jquery'), '20180213', true );
	...
	wp_enqueue_style( 'consultent-font-awesome', get_template_directory_uri() . '/fonts/font-awesome.min.css' );
...
add_action( 'wp_enqueue_scripts', 'consultent_scripts' );

// Enqueue Bootstrap
function consultent_enqueue_bootstrap() {
	wp_enqueue_style( 'consultent-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true );
}
add_action( 'wp_enqueue_scripts', 'consultent_enqueue_bootstrap', 9 );



/*--- Register widget area ---*/
add_action( 'widgets_init', 'consultent_widgets_init' );
function consultent_widgets_init() {
    register_sidebar( array(
        'name' => __( 'Main Sidebar', 'consultent-ex' ),
        'id' => 'sidebar-1',
        'description' => __( 'Widgets in this area will be shown on all posts and pages.', 'consultent-ex' ),
	...
	for ($i=1; $i<=3; $i++){
		register_sidebar( array(
			'name' => __( 'Footer-widget-area-', 'consultent-ex' ) . $i,
			'id' => 'footer-widget-area' . $i,
			'description' => __( 'Widgets in this area will be shown on footer.', 'consultent-ex' ),
			'before_widget' => '<li id="%1$s" class="widget %2$s">',
			'after_widget'  => '</li>',
			'before_title'  => '<h3 class="widgettitle">',
			'after_title'   => '</h3>',
...
function consultent_edit_link() {
	edit_post_link(
		sprintf( __( '<i class="fa fa-edit"></i>Edit<span class="screen-reader-text"> "%s"</span>', 'consultent-ex' ), get_the_title() ),
...
function consultent_get_breadcrumb() {
    echo '<a href="'.home_url().'" rel="nofollow">'.__('Home', 'consultent-ex').'</a>';
    if ( is_category() || is_single() ) {
        ...
        echo __("&nbsp;&nbsp;&#47;&nbsp;&nbsp;Search Results for... ", "consultent");
        echo '"<em>';
...
function consultent_recommended_plugin() {
...
add_action( 'tgmpa_register', 'consultent_recommended_plugin' );
...
add_filter( 'pre_set_site_transient_update_themes', 'consultent_check_for_update' );
function consultent_check_for_update( $checked_data ){
...
popularfx_maybe_promo([
	...
	'rating' => 'https://popularfx.com/themes/wordpress/corporate/Consultent-Ex',
